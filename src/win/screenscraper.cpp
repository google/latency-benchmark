...
BOOL CALLBACK init_start_time(PINIT_ONCE ignored, void *ignored2, void **ignored3) {
    BOOL r;
...
int64_t get_nanoseconds_elapsed(const LARGE_INTEGER &current_time) {
    BOOL r = InitOnceExecuteOnce(&start_time_init_once, &init_start_time, NULL, NULL);
    assert(r);
    int64_t elapsed = current_time.QuadPart - start_time.QuadPart;
    if (elapsed < 0) {
        elapsed = 0;
    }
    // We switch to floating point here before the division to avoid overflow and/or loss of precision.
    // We can't do this in 64-bit signed integer math: doing the multiplication first would overflow in
    // a matter of seconds, while doing the division first risks loss of accuracy if the timer resolution
    // is close to the nanosecond range.
    double elapsed_ns = elapsed * (nanoseconds_per_second / (double)frequency.QuadPart);
    return (int64_t) elapsed_ns;
}


// Returns the number of nanoseconds elapsed since the first call to get_nanoseconds in this process. The first
...
BOOL CALLBACK create_device(PINIT_ONCE ignored, void *ignored2, void **ignored3) {
...
#endif
    hr = D3D11CreateDevice(dxgi_adapter, D3D_DRIVER_TYPE_UNKNOWN, NULL, flags, levels, 1, D3D11_SDK_VERSION, &device, &out_level, &context);
    assert(hr == S_OK);
...
static screenshot *take_screenshot_with_dxgi(uint32_t x, uint32_t y, uint32_t width, uint32_t height) {
    HRESULT hr = S_OK;
    BOOL r = InitOnceExecuteOnce(&directx_initialization, &create_device, NULL, NULL);
    assert(r);
    ...
    while(true) {
        hr = dxgi_output_duplication->AcquireNextFrame(INFINITE, &frame_info, &screen_resource);
        if (hr == S_OK && frame_info.AccumulatedFrames > 0) {
            // A new screenshot was taken.
            if (frame_info.AccumulatedFrames > 1) {
                debug_log("more than one frame at a time: %d frames", frame_info.AccumulatedFrames);
            }
            int64_t new_screenshot_time = get_nanoseconds_elapsed(frame_info.LastPresentTime);
            last_screenshot_time = new_screenshot_time;
            break;
        } else if (hr != S_OK) {
            // TODO: This happens if the screensaver came on or the computer went to sleep. Try recreating the output duplication interface before giving up.
    ...
    hr = screen_resource->QueryInterface(__uuidof(ID3D11Texture2D), (void **)&framebuffer);
    assert(hr == S_OK);
    ...
    if (x >= framebuffer_desc.Width || y >= framebuffer_desc.Height || width == 0 || height == 0) {
    ...
    screenshot_desc.CPUAccessFlags = D3D11_CPU_ACCESS_READ | D3D11_CPU_ACCESS_WRITE;
    screenshot_desc.Format = DXGI_FORMAT_B8G8R8A8_UNORM;
    screenshot_desc.MipLevels = screenshot_desc.ArraySize = screenshot_desc.SampleDesc.Count = 1;
    ...
    box.front = 0;
    box.back = 1;
    context->CopySubresourceRegion(screenshot_texture, 0, 0, 0, 0, framebuffer, 0, &box);
    hr = context->Map(screenshot_texture, 0, D3D11_MAP_READ_WRITE, 0, &screenshot_mapped);
    assert(hr == S_OK);
...
static screenshot *take_screenshot_with_gdi(uint32_t x, uint32_t y, uint32_t width, uint32_t height) {
    HRESULT hr;
    int ir;
    BOOL r;
    // DWM causes screenshotting with GDI to be very slow. The only fix is to disable DWM during the test. DWM will automatically be re-enabled when the process exits. This API stops working on Windows 8 (though it returns success regardless).
    if (!composition_disabled) {
        // HACK: DwmEnableComposition fails unless GetDC(NULL) has been called first. Who knows why.
    ...
    uint8_t *pixels = NULL;
    HBITMAP hbitmap = CreateDIBSection(screen_dc, &bitmap_info, DIB_RGB_COLORS, (void **)&pixels, NULL, 0);
    assert(hbitmap);
    SelectObject(memory_dc, hbitmap);
    r = BitBlt(memory_dc, 0, 0, width, height, screen_dc, virtual_x, virtual_y, SRCCOPY);
    assert(r);
...
static bool use_dxgi() {
    ...
    assert(r);
    bool windows_8_or_greater = version.dwMajorVersion > 6 || (version.dwMajorVersion == 6 && version.dwMinorVersion >= 2);
...
bool open_browser(const char *url) {
    // Recommended by http://msdn.microsoft.com/en-us/library/windows/desktop/bb762153(v=vs.85).aspx
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
        if (paint_time - last_paint_time > max_paint_pause) {
            ...
            always_log("new max_paint_pause: %f", max_paint_pause / (double)nanoseconds_per_millisecond);
        }
        ...
        for (int i = 0; i < pattern_bytes; i += 4) {
            SetPixel(ps.hdc, i / 4, 0, RGB(test_pattern[i + 2], test_pattern[i + 1], test_pattern[i + 0]));
        }
        EndPaint(hwnd, &ps);
        break;
    default:
...
DWORD WINAPI message_loop(LPVOID ignored) {
    ...
    control_window = CreateWindow(window_class_name, "Web Latency Benchmark test window", WS_OVERLAPPEDWINDOW, 100, 100, 500, 500, NULL, NULL, window_class.hInstance, NULL);
    assert(control_window);
    ShowWindow(control_window, SW_SHOWNORMAL);
    SetWindowPos(control_window, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
