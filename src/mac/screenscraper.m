...
static const CGWindowImageOption image_options = kCGWindowImageBestResolution | kCGWindowImageShouldBeOpaque;

// width and height are automatically clamped to the screen, so to take a full-screen screenshot simply pass UINT32_MAX. On a retina display, all coordinates are in units of pixels in the backing store, not logical pixels. However, due to deficiencies of CGWindowListCreateImage, screenshots can only be taken at exact logical pixels. This means that on a retina display this function may fail if x or y don't correspond to an exact logical pixel. width and height will simply be rounded up to the next logical pixel.
screenshot *take_screenshot(uint32_t x, uint32_t y, uint32_t width, uint32_t height) {
    ...
    CGRect capture_rect = { .origin.x = x, .origin.y = y, .size.width = width, .size.height = height };
    // Clamp to the screen.
    capture_rect = CGRectIntersection(capture_rect, screen_rect);
    // Convert to Retina logical pixels from backing store pixels.
    ...
    if (!near_integer(converted_capture_rect.origin.x) || !near_integer(converted_capture_rect.origin.y)) {
        debug_log("Can't take screenshot at odd coordinates on a Retina display.");
        return NULL;
    }
    // Round width/height up to the next logical pixel.
    converted_capture_rect.size.width = ceilf(converted_capture_rect.size.width);
    converted_capture_rect.size.height = ceilf(converted_capture_rect.size.height);
    // Update capture_rect with the final rounded values.
    capture_rect = [screen convertRectToBacking:converted_capture_rect];
    CGImageRef window_image = CGWindowListCreateImage(converted_capture_rect, kCGWindowListOptionAll, kCGNullWindowID, image_options);
    ...
    bool correct_byte_order = (bitmap_info & kCGBitmapByteOrderMask) == kCGBitmapByteOrder32Little;
    bool correct_alpha_location = bitmap_info & kCGBitmapAlphaInfoMask & (kCGImageAlphaFirst | kCGImageAlphaNoneSkipFirst | kCGImageAlphaPremultipliedFirst);
    if (bpp != 32 || bpc != 8 || !correct_byte_order || !correct_alpha_location) {
        debug_log("Incorrect image format from CGWindowListCreateImage. bpp = %d, bpc = %d, byte order = %s, alpha location = %s", bpp, bpc, correct_byte_order ? "correct" : "wrong", correct_alpha_location ? "correct" : "wrong");
        CFRelease(window_image);
        return NULL;
    }
    CFDataRef image_data = CGDataProviderCopyData(CGImageGetDataProvider(window_image));
    CFRelease(window_image);
...
bool send_scroll_down(x, y) {
    CGFloat devicePixelRatio = [[[NSScreen screens] objectAtIndex:0] backingScaleFactor];
    CGWarpMouseCursorPosition(CGPointMake(x / devicePixelRatio, y / devicePixelRatio));
    static CGEventRef down = NULL;
    if (!down) {
